    ...
    $.fn.uploader = function(options) {
        ...
        ! function onLoad() {
            ...
            if (settings.inputCount == 0) {
                $(settings.wpFile).parent().append("<input type='hidden' id='deleted-files-" + settings.wpFile.attr('id') + "' name='deleted_files_" + settings.wpFile.attr('id') + "'>");
                $(settings.wpFile).append("<input type='file' id='input-" + settings.wpFile.attr('id') + "-" + settings.inputCount + "' name='files_" + settings.wpFile.attr('id') + "_[]' " + multiple + ">");
                settings.inputCount++;
            }
        ...
        function removerFile() {
            ...
            $('.thumbnail-cancel').each(function() {
                $(this).on('click', function() {
                    ...
                    var keyid = $(this).parent().parent().parent().parent().parent().find('.wp-uploader-banner').attr('id');
                    $('#input-' + keyid + '-' + this.id).remove();
                    ...
                    $('#deleted-files-' + keyid).val(JSON.stringify(arrDeletedFiles));
                    settings.currentTotalFile--;
                });
            });
        }
        ...
        function addTemplate(src, filePreviewElement, inputCount, fileName, fileType) {
            var keyid = $(filePreviewElement).attr('id');
            var template = "<div class='wp-thumbnail'>"+
                                "<div class='thumbnail'>"+
                                    "<a href='#'><img class='image-" + keyid + '-' + inputCount + "' src='' alt=''></a>"+
                                    "<div class='caption'>"+
                                    ...
                                    "</div>"+
                                "</div>"+
                                "<input type='text' name='url[]' class='form-control' placeholder='Url banner...'>"+
                            "</div>";
            ...
            $('.image-' + keyid + '-' + inputCount).attr('src', src);
        }
